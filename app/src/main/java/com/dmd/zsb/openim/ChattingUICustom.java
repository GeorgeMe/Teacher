...
public class ChattingUICustom extends IMChattingPageUI {
    ...
    public int getMsgBackgroundResId(YWConversation conversation, YWMessage message, boolean self) {
        ...
        if (msgType == YWMessage.SUB_MSG_TYPE.IM_TEXT || msgType == YWMessage.SUB_MSG_TYPE.IM_AUDIO){
            if (self){
                return R.drawable.open_im_talk_pop_r_bg;
            } else {
                return R.drawable.open_im_talk_pop_l_bg;
            }
        } else if (msgType == YWMessage.SUB_MSG_TYPE.IM_IMAGE){
            if (self){
                return R.drawable.open_im_talk_pic_pop_r_bg;
            } else {
                return R.drawable.open_im_talk_pic_pop_l_bg;
            }
...
//        }
    ...
    public View getCustomTitleView(final Fragment fragment,
        ...
        View view = inflater.inflate(R.layout.open_im_custom_chatting_title, new RelativeLayout(context), false);
        view.setBackgroundColor(Color.parseColor("#097cef"));
...
//                + "/alibaba/WXOPENI/云旺相册";
    ...
    private void receiveTribeMsg(YWTribe tribe, final View view) {
        LoginHelper.getInstance().getIMKit().getIMCore().getTribeService().unblockTribe(tribe, new IWxCallback() {
            @Override
